# Cross.DataFilter — GitHub Ruleset recipes
...
`Cross.DataFilter` is a **public** repository, so repository rulesets are available without GitHub Pro.
...
4. Optionally run a back-merge to `dev` via `TAGTOKEN` (workflow bootstraps `origin/dev` from `master` if the branch does not exist yet).
